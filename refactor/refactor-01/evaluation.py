...
Includes EvaluationFramework, HallucinationGuard, and ConsistencyEnforcer.
"""
...
class EvaluationFramework:
    ...
    def evaluate(
        ...
        """Run evaluation checks on content."""
        ...
        for check in checks_to_run:
            ...
            if method:
                report = method(content, context or {})
                ...
                reports.append(EvaluationReport(
                    ...
                    score=0.0,
                    details=f"No implementation for check: {check}"
                ))
    ...
    def _check_completeness(self, content: str, context: Dict) -> EvaluationReport:
        ...
        return EvaluationReport(
            ...
            result=self._score_to_result(score, "completeness"),
            score=score,
            details=f"Completeness indicators: {indicators}",
            evidence=[k for k, v in indicators.items() if not v]
        )
    
    def _check_accuracy(self, content: str, context: Dict) -> EvaluationReport:
        """Check for accuracy indicators (requires context)."""
        if "expected" not in context:
            return EvaluationReport(
                check_name="accuracy",
                result=EvaluationResult.SKIP,
                score=0.0,
                details="No expected content provided for accuracy check"
            )
        ...
        return EvaluationReport(
            check_name="accuracy",
            result=self._score_to_result(overlap, "accuracy"),
            score=overlap,
            details=f"Word overlap: {overlap:.2%}"
        )
    ...
    def _check_consistency(self, content: str, context: Dict) -> EvaluationReport:
        ...
        return EvaluationReport(
            ...
            result=self._score_to_result(score, "consistency"),
            score=score,
            details=f"Found {len(issues)} potential inconsistencies",
    ...
    def _check_safety(self, content: str, context: Dict) -> EvaluationReport:
        ...
        unsafe_patterns = [
            r'\b(password|secret|api[_-]?key)\s*[:=]',
            r'\b(exec|eval)\s*\(',
            r'rm\s+-rf\s+/',
        ]
        ...
        return EvaluationReport(
            check_name="safety",
            result=self._score_to_result(score, "safety"),
            score=score,
            details="Safety check for sensitive patterns",
            evidence=findings
        )
    ...
    def _check_relevance(self, content: str, context: Dict) -> EvaluationReport:
        ...
        if not query:
            return EvaluationReport(
                ...
                result=EvaluationResult.SKIP,
                score=0.0,
                details="No query provided for relevance check"
            )
        ...
        return EvaluationReport(
            check_name="relevance",
            result=self._score_to_result(relevance, "relevance"),
            score=relevance,
            details=f"Query word coverage: {relevance:.2%}"
        )
    ...
    def _check_formatting(self, content: str, context: Dict) -> EvaluationReport:
        ...
        indicators = {
            ...
            "no_trailing_ws": not re.search(r'\s+$', content, re.MULTILINE),
        }
        score = sum(indicators.values()) / len(indicators)
        
        return EvaluationReport(
            check_name="formatting",
            result=self._score_to_result(score, "formatting"),
            score=score,
            details=f"Formatting indicators: {indicators}"
        )
...
class HallucinationGuard:
    ...
    def check(self, content: str, context: Optional[Dict] = None) -> Tuple[bool, List[str]]:
        ...
        Returns (is_clean, list of flagged items).
        """
        flags = []
    ...
    def sanitize(self, content: str, flags: List[str]) -> str:
        """Add uncertainty markers to flagged content."""
        sanitized = content
        for flag in flags:
...
class ConsistencyEnforcer:
    ...
    def __init__(self):
        self._assertions: Dict[str, str] = {}  # topic -> assertion
        self._history: List[Dict[str, Any]] = []
    
    def record(self, topic: str, assertion: str) -> None:
        """Record an assertion for a topic."""
    ...
    def check(self, topic: str, new_assertion: str) -> Tuple[bool, Optional[str]]:
        """Check if new assertion is consistent with previous.
        
        Returns (is_consistent, previous_assertion or None).
        """
        ...
        prev_negated = bool(prev_words & negation_words)
        new_negated = bool(new_words & negation_words)
        
        if prev_negated != new_negated:
            return False, previous
        
        return True, previous
    
    def enforce(self, content: str) -> str:
        """Enforce consistency by checking content against history."""
        ...
        for topic in self._assertions:
            if topic.lower() in words:
                ...
                if not is_consistent:
                    logger.warning(f"Inconsistency detected for {topic}: previous='{previous}'")
        return content
